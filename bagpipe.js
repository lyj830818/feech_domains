...
eventEmitter.addListener('event-new-domain' , function( domain){
    bagpipe.push( crawer.oneurl, domain , crawer.getHandler);
...
crawer.getHandler = function (err, response, body) {

    //var url = response.request.uri.href;
    //console.dir(url);
    if (err) {
        eventEmitter.emit('event-error', 'error get ' +' ' + err);
        return;
    }
...
crawer.start = function(){
    _.each(START_URL, function( url){
        crawer.oneurl( url , crawer.getHandler);
    });
}
